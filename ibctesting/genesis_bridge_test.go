...
func (s *transferGenesisSuite) TestIRO() {
	...
	// create IRO plan
	_, err := s.hubApp().IROKeeper.CreatePlan(s.hubCtx(), amt, time.Hour, time.Now(), true, rollapp, irotypes.DefaultBondingCurve(), irotypes.DefaultIncentivePlanParams(), irotypes.DefaultParams().MinLiquidityPart, time.Hour, 0)
	s.Require().NoError(err)
