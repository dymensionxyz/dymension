package cli

import (
	"fmt"

	"github.com/spf13/cobra"

	"github.com/cosmos/cosmos-sdk/client"
	"github.com/cosmos/cosmos-sdk/client/flags"

	hyputil "github.com/bcp-innovations/hyperlane-cosmos/util"
	"github.com/dymensionxyz/dymension/v3/x/bridgingfee/types"
)

// GetQueryCmd returns the cli query commands for this module
func GetQueryCmd(queryRoute string) *cobra.Command {
	cmd := &cobra.Command{
		Use:                        types.ModuleName,
		Short:                      fmt.Sprintf("Querying commands for the %s module", types.ModuleName),
		DisableFlagParsing:         true,
		SuggestionsMinimumDistance: 2,
		RunE:                       client.ValidateCmd,
	}

	cmd.AddCommand(
		CmdQueryFeeHook(),
		CmdQueryFeeHooks(),
		CmdQueryAggregationHook(),
		CmdQueryAggregationHooks(),
	)

	return cmd
}

func CmdQueryFeeHook() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "fee-hook [hook-id]",
		Short: "Query a fee hook by ID",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
				return err
			}

			queryClient := types.NewQueryClient(clientCtx)

			hookId, err := hyputil.HexAddressFromString(args[0])
			if err != nil {
				return err
			}

			res, err := queryClient.FeeHook(cmd.Context(), &types.QueryFeeHookRequest{
				Id: hookId,
			})
			if err != nil {
				return err
			}

			return clientCtx.PrintProto(res)
		},
	}

	flags.AddQueryFlagsToCmd(cmd)
	return cmd
}

func CmdQueryFeeHooks() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "fee-hooks",
		Short: "Query all fee hooks",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
				return err
			}

			queryClient := types.NewQueryClient(clientCtx)

			res, err := queryClient.FeeHooks(cmd.Context(), &types.QueryFeeHooksRequest{})
			if err != nil {
				return err
			}

			return clientCtx.PrintProto(res)
		},
	}

	flags.AddQueryFlagsToCmd(cmd)
	return cmd
}

func CmdQueryAggregationHook() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "aggregation-hook [hook-id]",
		Short: "Query an aggregation hook by ID",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
				return err
			}

			queryClient := types.NewQueryClient(clientCtx)

			hookId, err := hyputil.HexAddressFromString(args[0])
			if err != nil {
				return err
			}

			res, err := queryClient.AggregationHook(cmd.Context(), &types.QueryAggregationHookRequest{
				Id: hookId,
			})
			if err != nil {
				return err
			}

			return clientCtx.PrintProto(res)
		},
	}

	flags.AddQueryFlagsToCmd(cmd)
	return cmd
}

func CmdQueryAggregationHooks() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "aggregation-hooks",
		Short: "Query all aggregation hooks",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
				return err
			}

			queryClient := types.NewQueryClient(clientCtx)

			res, err := queryClient.AggregationHooks(cmd.Context(), &types.QueryAggregationHooksRequest{})
			if err != nil {
				return err
			}

			return clientCtx.PrintProto(res)
		},
	}

	flags.AddQueryFlagsToCmd(cmd)
	return cmd
}