...
func (s *KeeperTestSuite) TestCreatePlan() {
	...
	planId, err := k.CreatePlan(s.Ctx, "adym", allocation, eq, time.Hour, time.Now(), true, false, rollapp, curve, incentives, liquidityPart, time.Hour, 0)
	s.Require().NoError(err)
	...
	planId2, err := k.CreatePlan(s.Ctx, "adym", allocation, eq, time.Hour, time.Now(), true, false, rollapp2, curve, incentives, liquidityPart, time.Hour, 0)
	s.Require().NoError(err)
...
func (s *KeeperTestSuite) TestStandardLaunch_TargetRaise_InUSDC() {
	...
	iroParams.StandardLaunch.InitialTvl = math.NewInt(100).MulRaw(1e6)                         // 100 USDC
