...
func (s *KeeperTestSuite) TestValidateRollappPreconditions() {
	...
	s.Run("MissingGenesisChecksum", func() {
		...
		_, err := k.CreatePlan(s.Ctx, allocation, time.Hour, time.Now(), true, rollapp, curve, incentives, liquidityPart, time.Hour, 0)
		s.Require().Error(err)
	})
...
func (s *KeeperTestSuite) TestCreatePlan() {
	...
	rollapp, _ := s.App.RollappKeeper.GetRollapp(s.Ctx, rollappId)
	planId, err := k.CreatePlan(s.Ctx, allocation, time.Hour, time.Now(), true, rollapp, curve, incentives, liquidityPart, time.Hour, 0)
	s.Require().NoError(err)
	...
	planId2, err := k.CreatePlan(s.Ctx, allocation, time.Hour, time.Now(), true, rollapp2, curve, incentives, liquidityPart, time.Hour, 0)
	s.Require().NoError(err)
