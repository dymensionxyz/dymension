...
func (s *KeeperTestSuite) TestBuy() {
	...
	planId, err := k.CreatePlan(s.Ctx, totalAllocation, startTime, startTime.Add(time.Hour), rollapp, curve, incentives, types.DefaultParams().MinLiquidityPart, time.Hour)
...
func (s *KeeperTestSuite) TestTradeAfterSettled() {
	...
	rollapp, _ := s.App.RollappKeeper.GetRollapp(s.Ctx, rollappId)
	planId, err := k.CreatePlan(s.Ctx, totalAllocation, startTime, endTime, rollapp, curve, incentives, types.DefaultParams().MinLiquidityPart, time.Hour)
	s.Require().NoError(err)
