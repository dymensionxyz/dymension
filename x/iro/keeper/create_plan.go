...
func (k Keeper) GetCurveByLiquidityDenom(ctx sdk.Context, liquidityDenom string, params types.StandardLaunch) (types.BondingCurve, math.Int, error) {
	...
	if err != nil {
		return types.BondingCurve{}, math.Int{}, dymerrors.Join(gerrc.ErrInvalidArgument, fmt.Errorf("failed to convert target raise to liquidity denom: %w", err))
	}
	...
	if err != nil {
		return types.BondingCurve{}, math.Int{}, dymerrors.Join(gerrc.ErrInvalidArgument, fmt.Errorf("failed to convert initial TVL to liquidity denom: %w", err))
	}
	...
	if err != nil {
		return types.BondingCurve{}, math.Int{}, dymerrors.Join(gerrc.ErrInvalidArgument, fmt.Errorf("failed to find graduation point: %w", err))
	}
...
func (k Keeper) convertTargetRaiseToLiquidityDenom(ctx sdk.Context, targetRaise sdk.Coin, liquidityDenom string) (sdk.Coin, error) {
	...
	if err != nil {
		return sdk.Coin{}, dymerrors.Join(gerrc.ErrInvalidArgument, fmt.Errorf("failed to convert target raise to base denom: %w", err))
	}
	...
	if err != nil {
		return sdk.Coin{}, dymerrors.Join(gerrc.ErrInvalidArgument, fmt.Errorf("failed to convert target raise to liquidity denom: %w", err))
	}
