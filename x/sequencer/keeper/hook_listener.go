...
func (hook rollappHook) AfterUpdateState(ctx sdk.Context, rollappId string, stateInfo *rollapptypes.StateInfo, lastStateUpdateBySequencer bool) error {
	proposer := hook.k.GetProposer(ctx, stateInfo.StateInfoIndex.RollappId)
	if proposer.Address != stateInfo.Sequencer {
