...
func networkWithSchedulerObjects(t *testing.T, n int) (*network.Network, []types.Scheduler) {
	t.Helper()
	...
	return network.New(t, cfg), state.SchedulerList
}

func TestShowScheduler(t *testing.T) {
	net, objs := networkWithSchedulerObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowScheduler(), args)
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetSchedulerResponse
...
func TestListScheduler(t *testing.T) {
	net, objs := networkWithSchedulerObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListScheduler(), args)
			require.NoError(t, err)
			var resp types.QueryAllSchedulerResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
