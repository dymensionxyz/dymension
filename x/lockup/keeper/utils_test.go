...
func TestLockRefKeys(t *testing.T) {
	...
	lock1 := types.NewPeriodLock(1, sdk.AccAddress{}, time.Second, time.Now(), sdk.Coins{math.NewInt64Coin("stake", 10)})
	_, err := lockRefKeys(lock1)
	require.Error(t, err)

	// empty address and 2 coins
	lock2 := types.NewPeriodLock(1, sdk.AccAddress{}, time.Second, time.Now(), sdk.Coins{math.NewInt64Coin("stake", 10), math.NewInt64Coin("atom", 1)})
	_, err = lockRefKeys(lock2)
	require.Error(t, err)

	// not empty address and 1 coin
	lock3 := types.NewPeriodLock(1, addr1, time.Second, time.Now(), sdk.Coins{math.NewInt64Coin("stake", 10)})
	keys3, err := lockRefKeys(lock3)
	require.NoError(t, err)
	require.Len(t, keys3, 8)

	// not empty address and empty coin
	lock4 := types.NewPeriodLock(1, addr1, time.Second, time.Now(), sdk.Coins{math.NewInt64Coin("stake", 10)})
	keys4, err := lockRefKeys(lock4)
	require.NoError(t, err)
	require.Len(t, keys4, 8)

	// not empty address and 2 coins
	lock5 := types.NewPeriodLock(1, addr1, time.Second, time.Now(), sdk.Coins{math.NewInt64Coin("stake", 10), math.NewInt64Coin("atom", 1)})
