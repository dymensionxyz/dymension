...
// After expiry date, if no one has placed a bid, this Sell-Order will be
// closed, no change.
//   - If there is a bid, the highest bid will win, and the Dym-Name/Alias
//     ownership will be transferred to the winner.
//   - If the bid matches the sell price, the Dym-Name/Alias ownership will be
//     transferred to the bidder immediately.
type SellOrder struct {
	...
	// min_price is the minimum price that the owner is willing to accept for the
	// asset.
	MinPrice types.Coin `protobuf:"bytes,4,opt,name=min_price,json=minPrice,proto3" json:"min_price"`
	// sell_price is the price that the owner is willing to sell the
	// Dym-Name/Alias for, the SO will be completed when the price is met,
	// ownership transferred. If the sell price is zero, the SO will be closed
	// when the expire_at is reached and the highest bidder wins.
	SellPrice *types.Coin `protobuf:"bytes,5,opt,name=sell_price,json=sellPrice,proto3" json:"sell_price,omitempty"`
	// highest_bid is the highest bid on the SO, if any. Price must be greater
	// than or equal to the min_price.
...
// When the owner of the Dym-Name/Alias accepts the offer, deposited amount will
// be transferred to the owner. When the buyer cancels the offer, deposited
...
type BuyOrder struct {
	...
	// counterparty_offer_price is the price that the Dym-Name/Alias owner is
	// willing to sell for. This is used for counterparty price negotiation and
	// for information only. The transaction can only be executed when the owner
	// accepts the offer with exact offer_price.
...
// ReverseLookupBuyOrderIds contains a list of Buy-Orders IDs for reverse
// lookup.
type ReverseLookupBuyOrderIds struct {
	// order_ids is a list of Buy-Order IDs of the Buy-Orders linked to the
